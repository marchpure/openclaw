...
import { createFileSessionStore, createAgentRegistry, AcpxRuntime } from "acpx/runtime";
import {
...
const VEFAAS_OPENCODE_ACP_BACKEND_ID = "vefaas-opencode";
...
export function createVefaasOpencodeAcpService(params: {
  ...
  return {
    ...
    async start(ctx) {
      if (!params.pluginConfig.opencodeAcp.enabled) {
        return;
      }
