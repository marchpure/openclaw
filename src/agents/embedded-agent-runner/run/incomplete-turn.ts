...
const POST_TOOL_CONTINUATION_ACTION_RE =
  /\b(?:create|write|add|implement|build|update|fix|finish|complete|test|verify|run|generate|prepare|wire|set up|start|continue)\b/i;
...
export function resolvePostToolContinuationRetryInstruction(params: {
  ...
  const intentToContinue =
    ...
    POST_TOOL_LEADING_PROGRESS_RE.test(text) ||
